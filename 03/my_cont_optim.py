...
EXP_ID = 'changed' # the ID of this experiment (used to create log names)
...
def alpha_addition_crossover(p1, p2):
...
class AdaptiveStepMutation:
    def __init__(self, initial_step_size, final_step_size, max_generation):
        self.initial_step_size = initial_step_size
        self.final_step_size = final_step_size
        self.max_generation = max_generation

    def __call__(self, ind):
        # Adjust step size linearly based on generation progress
        step_size = self.initial_step_size + (self.final_step_size - self.initial_step_size) * (current_generation / self.max_generation)
        return ind + step_size * np.random.normal(size=ind.shape)
...
if __name__ == '__main__':
    ...
    for fit_gen, fit_name in zip(fit_generators, fit_names):
        ...
        mutate_ind = AdaptiveStepMutation(1, 0.01, MAX_GEN)
        xover = functools.partial(crossover, cross=alpha_addition_crossover, cx_prob=CX_PROB)
